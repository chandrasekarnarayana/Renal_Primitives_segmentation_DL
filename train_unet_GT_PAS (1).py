...
# Train U-Net Deep Learning network for Glomerular Tuft segmentation on H&E stained renal biopsy images.
# Useful reference: http://www.andrewjanowczyk.com/pytorch-unet-for-digital-pathology-segmentation/

import torch
print("Torch version: ",torch.__version__)

dataname="pas-normal-glomerular-tuft-5X"
...
class Dataset(object):
    ...
    def convert_to_tensor_form(self, img_new):
        ...
        img_new = transforms.ToTensor()(np.array(img_new))
        return img_new
...
def augment(patch_size = patch_size):
    return Compose([
        ...
        ElasticTransform(always_apply=True, approximate=True, alpha=20, sigma=10,alpha_affine=0,border_mode=cv2.BORDER_CONSTANT),
        GridDistortion(num_steps=16, distort_limit=0.5, border_mode=cv2.BORDER_CONSTANT,always_apply=False, p=0.5),
    ])
...
best_loss_on_test= checkpoint['best_loss_on_test']
...
print("epoch: "+str(epoch)+" all lose: "+str(best_loss_on_test))
...
for epoch in range(epoch,num_epochs):
    ...
    for phase in phases: #iterate through both training and validation states
        ...
        print("Total Data to be loaded: ",len(dataLoader[phase]))
        for ii , (X, y, y_weight) in enumerate(dataLoader[phase]): #for each of the batches
            ...
            with torch.set_grad_enabled(phase == 'train'): #dynamically set gradient computation, in case of validation, this isn't needed
                ...
                #print("dynamically set gradient computation")
...
[img,mask,mask_weight]=dataset["val"][3]
