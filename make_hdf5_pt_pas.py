...
files=glob.glob('./pas-pt-data/*_mask_proximal.png') # create a list of the files, in this case we're only interested in files which have masks so we can use supervised learning
...
for phase in phases.keys(): #now for each of the phases, we'll loop through the files
    ...
    for pidi in phases[phase]: #now for each of the files
        ...
        for fname in glob.glob(f'./pas-pt-data/{pid}*_mask_proximal.png'): 
            ...
            for imgtype in imgtypes:
                if(imgtype=="img"): #if we're looking at an img, it must be 3 channel, but cv2 won't load it in the correct channel order, so we need to fix that
                    ...
                    print ("./pas-pt-data/"+os.path.basename(fname).replace("_mask_proximal.png",".png"))
                    io=cv2.cvtColor(cv2.imread("./pas-pt-data/"+os.path.basename(fname).replace("_mask_proximal.png",".png")),cv2.COLOR_BGR2RGB)
